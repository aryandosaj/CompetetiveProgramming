...
struct compare
{
    bool operator()(const pair<ll, pair<ll, ll>> &lhs, const pair<ll, pair<ll, ll>> &rhs) const
    {
        if(lhs.second.first==-1||lhs.second.second==-n)
        {
            ll d1 = lhs.first-1;
            ll d2 = (rhs.first&1)?rhs.first/2:(rhs.first/2-1);
            if(d1==d2)
            {
        ...
        else if(rhs.second.first==-1||rhs.second.second==-n)
        {
            ll d2 = rhs.first-1;
            ll d1 = (lhs.first&1)?lhs.first/2:(lhs.first/2-1);
            if(d1==d2)
            {
                return lhs.second.first>rhs.second.first;
            }
        ...
        if (lhs.first == rhs.first)
                return lhs.second.first > rhs.second.first;
        if (abs(lhs.first - rhs.first) <= 1)
        {
            
            if (abs((lhs.first / 2) - (rhs.first / 2)) == 1)
            {
                return lhs.second.first > rhs.second.first;
            }
            else
...
set<pair<ll, pair<ll, ll>>,compare> s1;
set<pair<ll, pair<ll, ll>>>s2,s3;
map<ll, ll> mp, rmp;
    ...
    while (q--)
    {
        ...
        if (ty == 2)
        {
            ...
            else if (mp[banda - 1] != 1 && mp[banda + 1] == 1)
            {
                ...
                s3.erase(p);
                s1.erase({p.S.S, {-p.S.F, -p.F}});
                s2.erase({p.S.F, {p.F, p.S.S}});
            ...
            else if (mp[banda - 1] == 1 && mp[banda + 1] != 1)
            {
                ...
                s2.erase(p);
                s1.erase({p.S.S, {-p.F, -p.S.F}});
                s3.erase({p.S.F, {p.F, p.S.S}});
            ...
            else
            {
                ...
                s3.erase(p1);
                s1.erase({p1.S.S, {-p1.S.F, -p1.F}});
                s2.erase({p1.S.F, {p1.F, p1.S.S}});

                s2.erase(p2);
                s1.erase({p2.S.S, {-p2.F, -p2.S.F}});
                s3.erase({p2.S.F, {p2.F, p2.S.S}});

                //////////

                ll l = p1.S.F, r = p2.S.F, sz = p1.S.S + p2.S.S + 1;
                s1.insert({sz, {-l, -r}});
                s2.insert({l, {r, sz}});
                s3.insert({r, {l, sz}});
            }
